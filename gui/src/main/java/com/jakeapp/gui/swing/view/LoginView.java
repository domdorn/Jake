...
public class LoginView extends JXPanel implements Observer {
	...
	//	@Override
	public void contextChanged(EnumSet<ContextChangedCallback.Reason> reason, Object context) {
		if (reason.contains(ContextChangedCallback.Reason.MsgService)) {
			updateView();
		}
	}
...
//										Image.SCALE_SMOOTH));
	...
	private void initPredefinedCredentials() {
		for (SupportedServices service : SupportedServices.values()) {
			creds.put(service, controller.getPredefinedServiceCredentials(service.toString()));
		}
	}
...
//		EventCore.getInstance().addContextChangedListener(this); // todo reimplement this ?
	}
	...
	public void signInRegisterButtonPressed() {
		...
		if (isSignInRegisterButtonEnabled()) {
			...
			if (isModeSignIn()) {
				...
				try {
					// sync call
					MsgService msg = controller.addAccount(getCredentials());
	...
	private class RegisterAccountTask extends AbstractTask<Void> {
		...
		protected AvailableLaterObject<Void> calculateFunction() {
			...
			try {
				return controller.createAccount(cred);
			} catch (FrontendNotLoggedInException e) {
				log.warn(e);
				ExceptionUtilities.showError(e);
